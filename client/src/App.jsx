...
function MaintenanceCheck({ children }) {
    ...
    async function checkMaintenance() {
      try {
        ...
        if (res.ok) {
          ...
          if (mounted) setMaintenanceMode(data.enabled);
        } else {
  ...
  if (loading) return <LoadingFallback />;
  if (maintenanceMode === null) return <LoadingFallback />;
...
function ProtectedRoute({
  ...
  const isCommunityMember = ["alumni", "resident"].includes(user.role);
  ...
  if (requireChatAccess && !canAccessSyncChat && !isAdmin) return <Navigate to="/mentorship" replace />;
  if (requireCommunityMember && !isCommunityMember && !isAdmin) return <Navigate to="/collaboration" replace />;
          ...
          <Suspense fallback={<LoadingFallback />}>
            <Routes>
              ...
              <Route
                ...
                element={
                  <MaintenanceCheck>
                    <ProtectedRoute disallowAdmin={true} requireIntern={true}>
                      <Dashboard />
                    </ProtectedRoute>
                  </MaintenanceCheck>
                }
