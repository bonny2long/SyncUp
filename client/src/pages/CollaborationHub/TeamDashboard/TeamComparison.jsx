...
const TeamComparison = ({ data }) => {
  const comparisonData = useMemo(() => {
    ...
    return data
      .map((member) => ({
        ...
        performance:
          member.avg_team_signals > 0 ?
            (
              ((member.user_signals - member.avg_team_signals) /
                member.avg_team_signals) *
              100
            ).toFixed(1)
          : 0,
      }))
  ...
  const getPerformanceIcon = (performance) => {
    if (performance > 20) return <Rocket className="w-4 h-4" />;
    if (performance < -20)
      return <TrendingUp className="w-4 h-4 rotate-180 text-red-500" />;
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
        ...
        <p className="text-gray-500 text-sm">
          Individual team member performance data will appear here once signals
          are generated.
        </p>
      </div>
    );
  ...
  const teamAverage =
    comparisonData.reduce((sum, member) => sum + member.userWeight, 0) /
    comparisonData.length;
  const topPerformer = comparisonData[0];
  const totalTeamWeight = comparisonData.reduce(
    (sum, member) => sum + member.userWeight,
    0,
  );
              ...
              >
                ...
                <td className="text-center py-3 px-4">
                  <span className="text-gray-600">
                    {Math.round(member.avgTeamSignals)}
                  </span>
                </td>
                <td className="text-center py-3 px-4">
                  <div
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${getPerformanceColor(member.performance)}`}
                  >
                    <span className="mr-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div className="flex items-start">
            ...
            <p className="text-gray-600">
              <strong>{topPerformer?.name}</strong> leads with{" "}
              {topPerformer?.userWeight} weight contribution
            </p>
          </div>
          <div className="flex items-start">
            <BarChart3 className="w-4 h-4 text-blue-500 mr-2" />
            <p className="text-gray-600 text-sm">
              <strong>Team average performance:</strong>{" "}
              <strong>{Math.round(teamAverage)}</strong> weight per member
            </p>
          </div>
          <div className="flex items-start">
            ...
            <p className="text-gray-600">
              {
                comparisonData.filter((m) => parseFloat(m.performance) > 0)
                  .length
              }{" "}
              members performing above average
            </p>
          </div>
          <div className="flex items-start">
            <TrendingUp className="w-4 h-4 text-green-500 mr-2" />
            <p className="text-gray-600 text-sm">
              <strong>{comparisonData.length}</strong> members generate{" "}
              <strong>{totalTeamWeight}</strong> skill weight
            </p>
          </div>
        </div>
      </div>
